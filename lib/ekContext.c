...
{
    ekMapDestroy(E, E->globals, NULL);
    ekArrayDestroy(E, &E->stack, NULL);
    ekArrayDestroy(E, &E->usedValues, ekValueDestroy);
...
static ekBool ekContextCreateObject(struct ekContext * E, ekFrame ** framePtr, ekValue * prototype, ekS32 argCount)
{
    ...
    if (initFunc) {
        return ekContextCall(E, framePtr, newObject, initFunc, argCount);
    }
...
static const char * ekValueDebugString(struct ekContext * E, ekValue * v)
{
    ...
    sprintf(buffer, "%s 0x%p %s", ekValueTypeName(E, v->type), v, valString);
    return buffer;
}
...
#endif /* ifdef EUREKA_TRACE_EXECUTION */
...
void ekContextLoop(struct ekContext * E, ekBool stopAtPop, ekValue * result)
{
    ...
    while (continueLooping && !E->error) {
        ...
        ekContextGC(E); // TODO: Not all the time
    }
}

static void ekMapEntryValueMark(struct ekContext * E, void * ignored, ekMapEntry * entry)
{
...
void ekContextGC(struct ekContext * E)
{
    ...
    for (i = 0; i < ekArraySize(E, &E->usedValues); i++) {
        E->usedValues[i]->used = ekFalse;
    }

    // -----------------------------------------------------------------------
    // mark all values used by things the VM still cares about

    ekMapIterateP1(E, E->globals, ekMapEntryValueMark, NULL);

    for (i = 0; i < ekArraySize(E, &E->frames); i++) {
        ekFrame * frame = E->frames[i];
        if (frame->thisVal) {
            ekValueMark(E, frame->thisVal);
        }
        if (frame->closure) {
            ekValueMark(E, frame->closure);
        }
        ekMapIterateP1(E, frame->locals, ekMapEntryValueMark, NULL);
    }

    for (i = 0; i < ekArraySize(E, &E->stack); i++) {
        ekValueMark(E, E->stack[i]);
    }
    ...
    for (i = 0; i < ekArraySize(E, &E->usedValues); i++) {
        ekValue * value = E->usedValues[i];
        if (!value->used) {
#ifdef EUREKA_TRACE_EXECUTION
            ekTraceExecution(("Sweeping value [type:%d] %p\n", value->type, value));
#endif
            ekValueDestroy(E, value);
            E->usedValues[i] = NULL; // for future squashing
        }
    }
    ekArraySquash(E, &E->usedValues);
}
